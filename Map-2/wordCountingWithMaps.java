...
public Map<String, Boolean> wordMultiple(String[] strings) {
  ...
  /*Marking a previously unseen key as false and then as true it's been
  marked false allows us to forgo having to make two hashmaps: one to accumulate
  the number of occurences and another hashmap w/ boolean values to return.*/
  for(String s:strings){
    if(!out.containsKey(s)){
      out.put(s,false);
    }else if(out.get(s)==false){
      out.replace(s,true);
    }else{
      continue;
    }
...
public String[] firstSwap(String[] strings) {
  ...
  for(String s:strings){
    ...
    if(!map.containsKey(c)){
      ...
      if(l.size()<2){
        l.add(i);
        String temp = strings[l.get(0)];
        strings[l.get(0)] = strings[l.get(1)];
        strings[l.get(1)] = temp;
      } else {
        continue;
      }
    }
